import { SlashCommandBuilder, ApplicationIntegrationType, InteractionContextType, MessageFlags, ActionRowBuilder, ButtonBuilder, ButtonStyle, } from "discord.js";
...
export default {
    data: new SlashCommandBuilder()
        ...
        .addChoices({ name: "😀 Friendly", value: "friendly" }, { name: "💼 Professional", value: "professional" }, { name: "✂️ Concise", value: "concise" }, { name: "📈 Expand", value: "expand" })))
        .addSubcommand((sub) => sub
        .setName("proofread")
    ...
    async execute(interaction) {
        ...
        try {
            ...
            const row = new ActionRowBuilder().addComponents(new ButtonBuilder()
                .setLabel(`${subcommand
                .charAt(0)
                .toUpperCase()}${subcommand.slice(1)}d in ${style}`)
                .setEmoji(emojis.magic)
                .setStyle(ButtonStyle.Secondary)
                .setDisabled(true)
                .setCustomId("rewrite"));
            await interaction.editReply({ content: output, components: [row] });
        }
        catch {
