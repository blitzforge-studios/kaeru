import { SlashCommandBuilder, ButtonBuilder, ActionRowBuilder, ButtonStyle, ChannelType, PermissionFlagsBits, underline, bold, MessageFlags, ApplicationIntegrationType, InteractionContextType, ContainerBuilder, TextDisplayBuilder, MediaGalleryBuilder, MediaGalleryItemBuilder, SeparatorBuilder, SeparatorSpacingSize, TextChannel, NewsChannel, } from "discord.js";
import { emojis } from "../../utilities/emojis.js";
import { basePermissions } from "../../utilities/permissions.js";
import { checkAppPermissions } from "../../handlers/index.js";
import { saveStaffRoleId, setupLoggingChannel, } from "../../utilities/database.js";
export default {
    ...
    async execute(interaction) {
        ...
        if (sub === "ticket") {
            ...
            const staffRoleId = interaction.options.getRole("staff_role", true).id;
            const banner = interaction.options.getAttachment("image");
            const sendingChannel = interaction.options.getChannel("channel", true);
            // Kanal tipi kontrolü
            ...
            const container = new ContainerBuilder()
                ...
                .addTextDisplayComponents(new TextDisplayBuilder().setContent(interaction.options.getString("description") ||
                `# ${emojis.button} Create a Ticket\nUse the button below to open a support thread.`))
                .addSeparatorComponents(new SeparatorBuilder()
                .setSpacing(SeparatorSpacingSize.Large)
                .setDivider(true))
                .addMediaGalleryComponents(new MediaGalleryBuilder().addItems(new MediaGalleryItemBuilder().setURL(banner?.url ??
            ...
            if (!guild.members.me.permissions.has(PermissionFlagsBits.ManageMessages)) {
                await interaction.followUp({
                    content: `## ${emojis.danger} ${underline("Recommending")}\nGrant ${bold("Manage Messages")} so staff can pin the button message.`,
