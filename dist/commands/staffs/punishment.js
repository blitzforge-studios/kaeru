import { SlashCommandBuilder, PermissionFlagsBits, MessageFlags, ApplicationIntegrationType, InteractionContextType, EmbedBuilder, time, userMention, } from "discord.js";
import { addWarning, checkLoggingChannel, checkWarnings, } from "../../utilities/database.js";
...
export default {
    data: new SlashCommandBuilder()
        .setName("punishment")
        .setDescription("Punish the member by timing them out. This will also give them a warning.")
        ...
        "pt-BR": "Pune o membro com um timeout. Isso também enviará um aviso para eles.",
        ...
        value: "breaking_tos",
        name_localizations: {
            "pt-BR": "Violação dos Termos de Serviço do Discord",
        ...
        value: "other",
        name_localizations: {
            "pt-BR": 'Outro (Por favor, use com o parâmetro "adicional")',
        ...
        .setName("duration")
        .setDescription("Set a custom duration as minutes (Only works with other reason)")
        .setNameLocalizations({
        "pt-BR": "duração",
        ro: "durată",
        el: "διάρκεια",
    ...
    async execute(interaction) {
        ...
        const extra = interaction.options.getString("additional_reason") ??
        ...
        try {
            const logId = await checkLoggingChannel(guild.id);
            if (logId) {
                ...
                if (warnings >= 4) {
                    ...
                    await interaction.editReply({
                        content: `${emojis.info} Member got ban because they received 4 warnings.`,
                    });
                    return;
                }
            }
            const existing = member.communicationDisabledUntilTimestamp;
            if (existing && existing > Date.now()) {
                await member.disableCommunicationUntil(expiryTime, `${reason} Moderated by ${interaction.user.username}`);
                await interaction.editReply({
                    content: `## ${emojis.timeout} Time-outed\n> **Target:** ${target}\n> **Duration:** ${time(expiryTime, "R")}\n> "${reason}"\n\nNow they have ${warnings} warnings.`,
                });
            }
            else {
                if (warnings === 1)
            ...
            await interaction.editReply({
                content: `${emojis.error} Are we sure they are not time-outed already?\n-# If you think something is not right, please contact **@neodevils**.`,
            });
        }
    },
};
