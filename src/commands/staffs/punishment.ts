...
export default {
	...
	async execute(interaction: ChatInputCommandInteraction): Promise<void> {
		if (!(await checkAppPermissions(interaction, basePermissions))) return;
		...
		if (reason === "other") reason = extra;
		else if (extra.trim()) reason = `${mainReason}\n> ${extra}`;
		else reason = mainReason;
		...
		try {
			...
			if (existing && existing > Date.now()) {
				await member.disableCommunicationUntil(
					expiryTime,
					`${reason} Moderated by ${interaction.user.username}`,
				);
				await interaction.editReply({
					content: `## ${
						emojis.timeout
				...
				if (warnings === 1) duration = 60;
				else if (warnings === 2) duration = 1440;
				else if (warnings >= 3) duration = 10080;
				await member.disableCommunicationUntil(
					expiryTime,
					`${reason} Moderated by ${interaction.user.username}`,
				);
