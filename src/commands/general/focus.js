...
export default {
	...
	async execute({ interaction }) {
		...
		if (subcommand === "set") {
			...
			return await interaction.reply({
				content: enable
					? `${emojis.reactions.kaeru.emphasize} You’re now in focus mode.`
					: `${emojis.reactions.kaeru.emphasize} Focus mode disabled.`,
				flags: MessageFlags.Ephemeral,
			});
		}
		...
		if (subcommand === "clear") {
			...
			return await interaction.reply({
				content: `${emojis.reactions.kaeru.thumbsup} Focus mode cleared for everyone. Happy!`,
				flags: MessageFlags.Ephemeral,
			});
		}
	},
